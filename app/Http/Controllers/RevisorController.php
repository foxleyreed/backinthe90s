...
class RevisorController extends Controller
{
    ...
    public function rejectArticle(Article $article){
        ...
        $article->save();
        return redirect(route('revisor.dashboard'))->with('message', '(article) rjected');
    }

    public function undoArticle(Article $article){
        $article->is_accepted = NULL;
        $article->save();
        return redirect(route('revisor.dashboard'))->with('message', '(article) restored to unrevisioned');
    }
}
